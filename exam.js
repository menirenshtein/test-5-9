...
function Mission1(obj) {
    const soldier = obj.commandStructure.chiefOfStaff;
    return `${soldier.rank} ${soldier.name} ${soldier.contact.phone} `
}
...
function Mission2(obj){
    let soldiers = obj.personnel.length
    return soldiers
}
...
function Mission3(NewDeployment, allUnitObj ){
    const oldMission = allUnitObj.currentDeployment;
    let newHistory = { eventDate : oldMission.startDate, eventDescription: oldMission.mission += "In" + oldMission.location}
    allUnitObj.history[allUnitObj.history.length +1] = newHistory
    allUnitObj.currentDeployment.mission = NewDeployment.mission
    allUnitObj.currentDeployment.startDate = NewDeployment.startDate
    return allUnitObj
}

console.log(Mission3( aa, militaryUnit ))
...
function Mission4(firearmObj, unitObj){
    const weapons = unitObj.equipment.firearms;
    weapons.forEach(firearm => {
     if(firearm.status === firearmObj.status){
        (firearm.status === firearmObj.status)? firearm.quantity += firearmObj.quantity: weapons[weapons.length +1] = firearmObj
     } 
...
console.log(Mission4(a, militaryUnit))


function Mission5(unitObj){
    const training = unitObj.trainingPrograms;
    let amountOfWeeks = 0;
    training.forEach(trad => {
        amountOfWeeks += trad.duration
